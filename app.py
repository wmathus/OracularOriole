...
def search():
    ...
    if search_type == "snp":
        ...
        cursor.execute("SELECT * FROM snp_genome WHERE gene_id = %s", (query,))  # Fix table name in sql! need to add the colunm 
    else:
        return render_template("index.html", table=None, graph_url=None)
    ...
    if results:   #follow the dcumentation
        table = "<table border='1'><tr><th>SNP ID</th><th>Gene</th><th>P-Value</th><th>Source</th></tr>"
        for row in results:
            table += f"<tr><td>{row['snp_id']}</td><td>{row.get('gene_id', 'N/A')}</td><td>{row['p_value']}</td>"
            table += f"<td><a href='{row.get('link', '#')}' target='_blank'>Source</a></td></tr>"
        table += "</table>"

    # Generate P-value graph as an example witch need to change for sats visualisation 
    graph_url = None
    if results:
        import sys
        import matplotlib
        matplotlib.use("Agg")  # Fix GUI issue on mac
        import matplotlib.pyplot as plt
        import io
        import base64
